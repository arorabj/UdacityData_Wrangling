...
os.environ['AWS_ACCESS_KEY_ID'] = config.get('AWS','AWS_KEY')
...
def process_song_data(spark, input_data, output_data):
    ...
    songs_table = df.select('song_id', 'title', 'artist_id', 'year', 'duration').dropDulicates()
...
def process_log_data(spark, input_data, output_data):
    ...
    df = df.filter(df.page == 'NextSong').select('ts', 'userId', 'level', 'song', 'artist', 'sessionId', 'location',
                                                 'userAgent')
    ...
    actions_df = df.withColumn('timestamp', get_timestamp(df.ts))

    # create datetime column from original timestamp column
    get_datetime = udf(lambda x: str(datetime.fromtimestamp(int(x) / 1000)))
    actions_df = actions_df.withColumn('timestamp', get_datetime(actions_df.ts))
...
def main():
    ...
    input_data = "s3a://anant-udacity-practice/"
    output_data = "s3a://anant-udacity-practice/output_data/"
